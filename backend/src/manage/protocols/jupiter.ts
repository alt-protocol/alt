...
function isEarnCategory(category: string): boolean {
  return (
    category === "earn" || category === "lending" || category === "supply"
  );
}

async function createEarnContext(
  ...
  const user = new sdk.PublicKey(walletAddress);
...
async function buildEarnDeposit(
...
async function buildEarnWithdraw(
  params: BuildTxParams,
): Promise<Instruction[]> {
  const ctx = await createEarnContext(params.walletAddress, params.extraData);
