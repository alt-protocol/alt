...
// MCP Server
// ---------------------------------------------------------------------------
const server = new McpServer({ name: "akashi", version: "1.0.0" }, { capabilities: { tools: {} } });
// --- Discover tools ---
server.tool("list_opportunities", "List Solana yield opportunities (stablecoin vaults, lending, earn). Returns APY, TVL, protocol, and category for each.", {
    ...
    try {
        ...
        if (stablecoins_only)
            params.set("stablecoins_only", "true");
...
server.tool("get_opportunity_details", "Get detailed information about a specific yield opportunity including APY history, deposit address, and protocol details.", {
...
// --- Monitor tools ---
server.tool("get_wallet_balance", "Get SPL token balances for a Solana wallet address.", {
...
server.tool("get_positions", "Get tracked DeFi positions for a wallet — deposit amounts, PnL, APY, and protocol details.", {
    wallet_address: z.string().describe("Solana wallet address"),
    ...
    try {
        ...
        apiPost(`/api/monitor/portfolio/${wallet_address}/track`, {}).catch((err) => console.error("Track wallet:", err.message));
...
// --- Manage tools ---
server.tool("build_deposit", "Build an unsigned deposit transaction for a yield opportunity. Returns serialized instructions and a simulation preview. The transaction must be signed locally before submitting.", {
...
server.tool("build_withdraw", "Build an unsigned withdrawal transaction for a yield opportunity. Returns serialized instructions and a simulation preview. The transaction must be signed locally before submitting.", {
    opportunity_id: z.number().int().positive().describe("Yield opportunity ID"),
    wallet_address: z.string().describe("Depositor's Solana wallet address"),
...
server.tool("submit_transaction", "Submit a signed Solana transaction to the network via Helius RPC. Returns the transaction signature.", {
